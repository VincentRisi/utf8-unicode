#include "prereqs_oci.h"
#define _VERSION_NO "$Revision: 412.1 $ $Date: 2004/11/18 10:27:35 $"
...
#if defined(M_W32)
int APPLAPI SqlVBLogon(pSqlCB *SqlCB, char* FileName, char* User)
{
...
int APPLAPI SqlNetLogon(pSqlCB *SqlCB, char* FileName, char* User)
{
  int RC = SqlLogon(SqlCB, FileName, User);
  (*SqlCB)->isVB = IS_NET;
  return RC;
...
int APPLAPI SqlLogon(pSqlCB *SqlCB, char* FileName, char* User)
{
...
int APPLAPI SqlOpen(pSqlCB SqlCB, pSqlCursor *SqlCursor, char* Query, void* Data)
{
  pSqlCursor Cursor;
  pSqlQuery  SqlQuery;
  if (InvalidCB(SqlCB))
...
int APPLAPI SqlOpenExec(pSqlCB SqlCB, pSqlCursor *SqlCursor, char* Query, void* Data)
{
  int RC;
  if (InvalidCB(SqlCB))
    return SqlCB->Error;
...
int APPLAPI SqlRunProc(pSqlCB SqlCB, char* Query, void* Data)
{
...
int APPLAPI SqlSetNull(pSqlCursor SqlCursor, char* aName, bool aHow)
{
  int i;
  for (i=0; i < SqlCursor->SqlQuery->NoFields; i++)
  {
...
int APPLAPI SqlTestNull(pSqlCursor SqlCursor, char* aName, bool *aHow)
{
  int i,n;
  for (i=0; i < SqlCursor->SqlQuery->NoFields; i++)
  {
...
int APPLAPI SqlFetchMany(pSqlCursor SqlCursor, unsigned short* aNo, void *Data)
{
  ushort i, No = *aNo;
...
char* APPLAPI SqlCBError(pSqlCB SqlCB)
{
  if (InvalidCB(SqlCB))
...
char* APPLAPI SqlCursorError(pSqlCursor SqlCursor)
{
  if (InvalidCursor(SqlCursor))
...
short APPLAPI SqlOneWayCipher(char* Input, ushort InpLen,

...
#define TEXT(x) x
#endif
#define KVSTORE_CONTROLLED_TRANSACTION_DEBUG

JP_INTERNAL(SQLRETURN) JP_Logon(TJConnector& conn, const char* User, const char* Password, const char* Server, const char* Schema = 0, const SQLHANDLE* cliEnv = 0, const char* connstring = 0);
...
JP_INTERNAL(SQLRETURN) JP_CheckConnect(TJConnector& conn, SQLLEN& attr);
...
JP_INTERNAL(SQLRETURN) JP_BindUtf8(TJQuery& qry, int32 bindNo, unsigned char* data, int32 dataLen, int32 direction);
JP_INTERNAL(SQLRETURN) JP_BindUtf8Null(TJQuery& qry, int32 bindNo, unsigned char* data, int32 dataLen, int16& nullChar, int32 direction);
JP_INTERNAL(SQLRETURN) JP_BindUnicode(TJQuery& qry, int32 bindNo, unsigned char* data, int32 dataLen, int32 direction);
JP_INTERNAL(SQLRETURN) JP_BindUnicodeNull(TJQuery& qry, int32 bindNo, unsigned char* data, int32 dataLen, int16& nullChar, int32 direction);
JP_INTERNAL(SQLRETURN) JP_BindBLOB(TJQuery& qry, int32 bindNo, char* data, int32 Datasize, int32 direction);
JP_INTERNAL(SQLRETURN) JP_BindLong(TJQuery& qry, int32 bindNo, SQLLEN& data, int32 direction);
...
JP_INTERNAL(SQLRETURN) JP_BindLongNull(TJQuery& qry, int32 bindNo, SQLLEN& data, int32 direction, int16& Null);
...
JP_INTERNAL(SQLRETURN) JP_BindCharArray(TJQuery& qry, int32 bindNo, char* data, int32 dataLen, SQLLEN* indicators);
JP_INTERNAL(SQLRETURN) JP_BindAnsiCharArray(TJQuery& qry, int32 bindNo, char* data, int32 dataLen, SQLLEN* indicators);
JP_INTERNAL(SQLRETURN) JP_BindInt64Array(TJQuery& qry, int32 bindNo, SQLLEN* data, SQLLEN* indicators);
JP_INTERNAL(SQLRETURN) JP_BindInt32Array(TJQuery& qry, int32 bindNo, int32* data, SQLLEN* indicators);
JP_INTERNAL(SQLRETURN) JP_BindInt16Array(TJQuery& qry, int32 bindNo, int16* data, SQLLEN* indicators);
JP_INTERNAL(SQLRETURN) JP_BindDoubleArray(TJQuery& qry, int32 bindNo, double* data, int32 precision, int32 scale, SQLLEN* indicators);
JP_INTERNAL(SQLRETURN) JP_BindDecimalArray(TJQuery& qry, int32 bindNo, char* data, int32 precision, int32 scale, SQLLEN* indicators);
JP_INTERNAL(SQLRETURN) JP_BindCLIDateArray(TJQuery& qry, int32 bindNo, DATE_STRUCT* data, SQLLEN* indicators);
JP_INTERNAL(SQLRETURN) JP_BindCLITimeArray(TJQuery& qry, int32 bindNo, TIME_STRUCT* data, SQLLEN* indicators);
JP_INTERNAL(SQLRETURN) JP_BindCLIDateTimeArray(TJQuery& qry, int32 bindNo, TIMESTAMP_STRUCT* data, SQLLEN* indicators);

JP_INTERNAL(SQLRETURN) JP_DefineString(TJQuery& qry, int32 defineNo, char* data, int32 dataLen, int32 typeChar);
JP_INTERNAL(SQLRETURN) JP_DefineUtf8(TJQuery& qry, int32 defineNo, unsigned char* data, int32 dataSize);
JP_INTERNAL(SQLRETURN) JP_DefineUnicode(TJQuery& qry, int32 defineNo, unsigned char* data, int32 dataSize);
JP_INTERNAL(SQLRETURN) JP_DefineBLOB(TJQuery& qry, int32 defineNo, char* data, int32 dataSize);
JP_INTERNAL(SQLRETURN) JP_DefineLong(TJQuery& qry, int32 defineNo, SQLLEN* data);
...
JP_INTERNAL(void) JP_GetLong(TJQuery& qry, SQLLEN& into, char* from);
...
JP_INTERNAL(void) JP_BigSequence(TJQuery& qry, SQLLEN& sequence, const char* sequencer);
JP_INTERNAL(void) JP_UserStamp(TJQuery& qry, char* userStamp);
JP_INTERNAL(void) JP_Date(TJQuery& qry, DATE_STRUCT& clidate, const char* date);
JP_INTERNAL(void) JP_Time(TJQuery& qry, TIME_STRUCT& clitime, const char* time);
JP_INTERNAL(void) JP_DateTime(TJQuery& qry, TIMESTAMP_STRUCT& cliDateTime, const char* dateTime);
...
JP_INTERNAL(int32) JP_JulianDate(const char* YYYYMMDD);
...
JP_INTERNAL(void) _Cancel(TJQuery& qry);
...
JP_INTERNAL(void) _SqlError(TJConnector& conn, int handleType);
...
JP_INTERNAL(void) _ToDateTime(TIMESTAMP_STRUCT& cliDateTime, int32 yyyymmdd, int32 hhmmss)
{
  ...
  cliDateTime.fraction = (SQLLEN)0;
}

bool TJAlloc::Failed = false;

void TJConnector::Logon(char* connstring, const SQLHANDLE* aCliEnv)
{
  Logon(0, 0, 0, 0, aCliEnv, connstring);
}

void TJConnector::Logon(const char* User, const char* Password, const char* Server, const char* Schema, const SQLHANDLE* cliEnv)
{
  Logon(User, Password, Server, Schema, cliEnv, 0);
}



void TJConnector::Logon(const char* User, const char* Password, const char* Server, const char* Schema, const SQLHANDLE* cliEnv, char* connstring)
{
  loggedOn = 0;
  if (JP_Logon(*this, User, Password, Server, Schema, cliEnv, connstring))
    ThrowOnError(JP_MARK);
#if defined(M_AIX) || defined(M_GNU)
  // Removed for semephore issues on redhat/static libs. 
  // timeout = getCliapiQueryTimeout();
  timeout = 0;
#endif
  loggedOn = 1;
}

void TJConnector::SetTimeout(unsigned int timeout)
{
#if defined(M_AIX) || defined(M_GNU)
...
bool TJConnector::StillConnected()
{
  if (loggedOn == 0)
    return false;
  SQLLEN attr = 0L;
  if (JP_CheckConnect(*this, attr))
    ThrowOnError(JP_MARK);
  bool result = (attr == SQL_CD_FALSE); // if CD (connection down) is false we return true
  return result;
}
...
void TJConnector::ThrowOnError(char const* file, int32 line)
{
  if (result != 0)
  {
    TOdbcApiException ex(result, state, allow, msgbuf, file, line);
    if (loggedOn != 0 && StillConnected() == false)
    {
      Logoff();
      loggedOn = 0;
#if defined(M_AIX) || defined(M_GNU)
      SLOG->lprintf(eLogError, "%s:%s is no longer connected to database. Raising a SIGUSR1 to terminate processing.", server, schema);
      raise(SIGUSR1);
#endif
    }
    throw ex;
...
void TJConnector::TimeStamp(char* timeStamp, int& fraction)
{
  ...
  if ((int)floor(serverTime) == 0)
  {
    ...
    {
    case CPP_RET_DB2:
      command = "SELECT CURRENT_TIMESTAMP from SYSIBM.SYSDUMMY1 WITH UR";
      break;
    case CPP_RET_ORACLE:
    ...
    TJQuery q(*this);
    q.command = TJAlloc::newChar(strlen(command) + 32);
...
void TJQuery::BindUtf8(int32 bindNo, unsigned char* data, int32 dataLen, int16 direction, int16* null)
{
  int32 result;
  size_t dlen = strlen((char*)data);
  if (null)
    result = JP_BindUtf8Null(*this, bindNo, data, dlen, direction, *null);
  else
    result = JP_BindUtf8(*this, bindNo, data, dlen, direction);
  if (result)
    conn.ThrowOnError(file, line);
}

#include <cwchar> 
void TJQuery::BindUnicode(int32 bindNo, unsigned char* data, int32 datalen, int16 direction, int16* null)
{
  int32 result;
  size_t wlen = wcslen((wchar_t*)data) * 2;
  if (null)
    result = JP_BindUnicodeNull(*this, bindNo, data, wlen, direction, *null);
  else
    result = JP_BindUnicode(*this, bindNo, data, wlen, direction);
  if (result)
    conn.ThrowOnError(file, line);
}

void TJQuery::BindBlob(int32 bindNo, unsigned char* data, int32 dataSize, int16 direction)
{
  int32 result;
  result = JP_BindBLOB(*this, bindNo, (char*)data, dataSize, direction);
  if (result)
    conn.ThrowOnError(file, line);
}
...
void TJQuery::Bind(int32 bindNo, SQLLEN& data, int16 direction, int16* null)
{
  if (null)
...
void TJQuery::BindCharArray(int32 bindNo, char* data, int dataLen, SQLLEN* null)
{
  conn.ThrowOnError(JP_BindCharArray(*this, bindNo, data, dataLen, null), file, line);
}

void TJQuery::BindAnsiCharArray(int32 bindNo, char* data, int dataLen, SQLLEN* null)
{
  conn.ThrowOnError(JP_BindAnsiCharArray(*this, bindNo, data, dataLen, null), file, line);
}

void TJQuery::BindInt64Array(int32 bindNo, SQLLEN* data, SQLLEN* null)
{
  conn.ThrowOnError(JP_BindInt64Array(*this, bindNo, data, null), file, line);
}

void TJQuery::BindInt32Array(int32 bindNo, int32* data, SQLLEN* null)
{
  conn.ThrowOnError(JP_BindInt32Array(*this, bindNo, data, null), file, line);
}

void TJQuery::BindInt16Array(int32 bindNo, int16* data, SQLLEN* null)
{
  conn.ThrowOnError(JP_BindInt16Array(*this, bindNo, data, null), file, line);
}

void TJQuery::BindDoubleArray(int32 bindNo, double* data, int16 precision, int16 scale, SQLLEN* null)
{
  conn.ThrowOnError(JP_BindDoubleArray(*this, bindNo, data, precision, scale, null), file, line);
}

void TJQuery::BindMoneyArray(int32 bindNo, char* data, int16 precision, int16 scale, SQLLEN* null)
{
  conn.ThrowOnError(JP_BindDecimalArray(*this, bindNo, data, precision, scale, null), file, line);
}

void TJQuery::BindDateArray(int32 bindNo, DATE_STRUCT* data, SQLLEN* null)
{
  conn.ThrowOnError(JP_BindCLIDateArray(*this, bindNo, data, null), file, line);
}

void TJQuery::BindTimeArray(int32 bindNo, TIME_STRUCT* data, SQLLEN* null)
{
  conn.ThrowOnError(JP_BindCLITimeArray(*this, bindNo, data, null), file, line);
}

void TJQuery::BindDateTimeArray(int32 bindNo, TIMESTAMP_STRUCT* data, SQLLEN* null)
{
...
{
  conn.ThrowOnError(JP_DefineUtf8(*this, defineNo, data, dataLen), file, line);
}

void  TJQuery::DefineUnicode(int32 defineNo, unsigned char* data, int32 dataLen) 
{
  conn.ThrowOnError(JP_DefineUnicode(*this, defineNo, data, dataLen), file, line);
}

void TJQuery::Define(int32 defineNo, SQLLEN* data)
{
  if (cppRetType == CPP_RET_ORACLE)
    conn.ThrowOnError(JP_DefineString(*this, defineNo, (char*)data, 18, 1), file, line);
  else
...
void TJQuery::Get(SQLLEN& into, char* from)
{
  if (cppRetType == CPP_RET_ORACLE)
  {
    char work[19];
    JP_GetString(*this, work, from, 18);
    into = atoll(work);
  }
  else
...
#if defined USE_TJBLOB_WITH_GETTERS
void  TJQuery::GetBlob(void* into, unsigned char* from, int32 dataLen)
{
  JP_GetBLOB(*this, into, from, dataLen);
}
#else
void  TJQuery::GetBlob(int32& intolen, unsigned char* into, char* from, int32 dataSize)
{
  JP_GetBLOB(*this, intolen, into, from, dataSize);
}
#endif
...
void TJQuery::Clear()
{
  ...
  if (noIndicators)
    memset(indicators, 0, noIndicators * sizeof(SQLLEN));
}
...
SQLLEN& TJQuery::Sequence(SQLLEN& sequence, char* sequencer)
{
...
TIMESTAMP_STRUCT& TJQuery::TimeStamp(TIMESTAMP_STRUCT& clitimestamp, char* timeStamp)
{
  ...
  string s = std::to_string(clitimestamp.fraction);
  int len = 6;
  s = s.substr(0, 3);
  while (len--)
    s.append("0");
  clitimestamp.fraction = stoi(s);

  return clitimestamp;
}

char* TJQuery::AsChar(char asChar[], SQLLEN i64)
{
  memset(asChar, 0, sizeof(asChar));
  snprintf(asChar, 18, "%lld", i64);
  return asChar;
}

CPP_RET_TYPE cppRetType;

void TJCppRet::setType(CPP_RET_TYPE aType) { cppRetType = aType; }
...
size_t TJCppRet::setup(const char* serverName, const char* tableName, const char* procName, const char* fieldName, bool hasReturning)
{
  ...
  case CPP_RET_DB2:
    if (hasReturning)
    {
      snprintf(work, sizeof(work), "select %s from new table (", fieldName);
      head = TJAlloc::dupChar(work);
      size += strlen(work);
    }
    snprintf(work, sizeof(work), "NEXTVAL FOR %sSEQ,", tableName);
  ...
  case CPP_RET_MSSQL:
    if (hasReturning)
    {
      snprintf(work, sizeof(work), "OUTPUT Inserted.%s", fieldName);
      output = TJAlloc::dupChar(work);
      size += strlen(work);
    }
    useField = false;
    return size;
  ...
  case CPP_RET_ORACLE:
    ...
    snprintf(work, sizeof(work), " %sseq.nextval,", tableName);
    sequence = TJAlloc::dupChar(work);
    size += strlen(work);
    if (hasReturning)
    {
    ...
    return size;
  case CPP_RET_POSTGRE:
    useField = false;
    strncpy(work, "0,", sizeof(work));
    sequence = TJAlloc::dupChar(work);
    size += strlen(work);
    if (hasReturning)
    {
...
JP_INTERNAL(void) _SqlError(TJConnector& conn, int handleType)
{
  ...
  if (conn.result != SQL_SUCCESS)
  {
    ...
    SQLLEN numRecs = 0;
    SQLGetDiagField(handleType, conn.diagHandle, 0, SQL_DIAG_NUMBER, &numRecs, 0, 0);
...
#if defined(M_AIX) || defined(M_GNU)
      if (conn.cancelled != 0 && strlen(conn.cancelled) + 1 < sizeof(conn.msgbuf) - strlen(conn.msgbuf))
      {
        strcat(conn.msgbuf, conn.cancelled);
        strcat(conn.msgbuf, "\n");
      }
#endif    
...
JP_INTERNAL(SQLRETURN) _Result(TJConnector& conn)
{
  _SqlError(conn, SQL_HANDLE_DBC);
  return conn.result;
}

JP_INTERNAL(SQLRETURN) _Result(TJQuery& qry)
{
  _SqlError(qry.conn, SQL_HANDLE_STMT);
...
JP_INTERNAL(SQLRETURN) JP_CheckConnect(TJConnector& conn, SQLLEN& attr)
{
  SQLLEN size = sizeof(attr);
  conn.ForDiagnosis(SQL_HANDLE_DBC, conn.cliDbc);
  conn.result = SQLGetConnectAttr(conn.cliDbc, SQL_ATTR_CONNECTION_DEAD, (SQLPOINTER)&attr, size, NULL);
  return _Result(conn);
}

JP_INTERNAL(SQLRETURN) JP_Logon(TJConnector& conn, const char* aUser, const char* aPassword, const char* aServer, const char* aSchema, const SQLHANDLE* aCliEnv, const char* connstring)
{
  if (aUser != 0)
  ...
  // we allocated a cliEnv handle even if we are not coordinated
  // if aCliEnv is passed in as not zero we mark ourselves not the owner
  // and expect to be part of two phase commits
  ...
  // if we have not had errors on the cliEnv route we do the setEnvAttr
  if (conn.Allow(conn.result) == true)
  {
    conn.ForDiagnosis(SQL_HANDLE_ENV, conn.cliEnv);
    conn.result = SQLSetEnvAttr(conn.cliEnv, SQL_ATTR_ODBC_VERSION, (void*)SQL_OV_ODBC3, 0);
  }
  if (conn.Allow(conn.result) == true)
  {
    conn.ForDiagnosis(SQL_HANDLE_ENV, conn.cliEnv);
...
#if defined USE_COORDINATED_IN_MSS
  // if we want coordinated we set it if this connection is the owner
  ...
  if (conn.Allow(conn.result) == true)
  {
    ...
    if (connstring != 0)
    {
      conn.result = SQLDriverConnect(conn.cliDbc, NULL, (SQLCHAR*)TEXT(connstring), SQL_NTS,
        NULL,
        0,
        NULL,
        SQL_DRIVER_NOPROMPT);
    ...
    else
    {
      ...
      conn.result = SQLConnect(conn.cliDbc, server.data, SQL_NTS, user.data, SQL_NTS, password.data, SQL_NTS);
    }
  }
  if (conn.Allow(conn.result) == true)
  { /* set AUTOCOMMIT OFF */
    conn.ForDiagnosis(SQL_HANDLE_DBC, conn.cliDbc);
    conn.result = SQLSetConnectAttr(conn.cliDbc, SQL_ATTR_AUTOCOMMIT, (SQLPOINTER)SQL_AUTOCOMMIT_OFF, SQL_NTS);
  }
  // We don't explicity set a session level schema in SQL Server
...
JP_INTERNAL(SQLRETURN) JP_SetForkable(TJConnector& conn)
{
#if defined USE_SETFORKABLE_IN_MSS
  if (conn.Allow(conn.result) == true)
  {
    conn.ForDiagnosis(SQL_HANDLE_ENV, conn.cliEnv);
...
#ifdef KVSTORE_CONTROLLED_TRANSACTION_DEBUG
    SLOG->lprintf(eLogDebug, "Coordinated Commit %s:%s", conn.server, conn.schema);
#endif
  }
  else
  {
...
#ifdef KVSTORE_CONTROLLED_TRANSACTION_DEBUG
    SLOG->lprintf(eLogDebug, "Commit %s:%s", conn.server, conn.schema);
#endif
  }
  return _Result(conn);
}
...
#ifdef KVSTORE_CONTROLLED_TRANSACTION_DEBUG
    SLOG->lprintf(eLogDebug, "Coordinated Rollback %s:%s", conn.server, conn.schema);
#endif
  }
  else
  {
...
#ifdef KVSTORE_CONTROLLED_TRANSACTION_DEBUG
    SLOG->lprintf(eLogDebug, "Rollback %s:%s", conn.server, conn.schema);
#endif
  }
  return _Result(conn);
}
...
#if defined(M_AIX) || defined(M_GNU)
static TJQuery* alarm_qry;
static void alarm_exec_break(int signal)
{
  strcpy(alarm_qry->conn.cancelled, "Query cancelled due to timeout");
  _Cancel(*alarm_qry);
}
struct autoAlarm
{
  unsigned int timeout;
  autoAlarm(TJQuery& qry)
  {
    timeout = qry.conn.timeout;
    strcpy(qry.conn.cancelled, "");
    if (timeout > 1000) timeout /= 1000;
    if (timeout > 0)
    {
      alarm_qry = &qry;
      signal(SIGALRM, alarm_exec_break);
      alarm(timeout);
    }
  }
  ~autoAlarm()
...
JP_INTERNAL(SQLRETURN) JP_Exec(TJQuery& qry)
{
  ...
  qry.conn.ForDiagnosis(SQL_HANDLE_STMT, qry.cliStmt);
#if defined(M_AIX) || defined(M_GNU)
  autoAlarm _alarm(qry); // cannot be placed in an if has to be in the same scope as the SQLExecute
#endif
...
JP_INTERNAL(void) JP_GetLong(TJQuery& qry, SQLLEN& into, char* from)
{
  into = *(SQLLEN*)(from + (qry.rowsIndex * sizeof(SQLLEN)));
}
...
JP_INTERNAL(SQLRETURN) JP_BindUtf8Null(TJQuery& qry, int32 bindNo, unsigned char* data, int32 dataLen, int16& nullChar, int32 direction)
{
  int16 null = strlen((char*)data) > 0 ? JP_NOT_NULL : JP_NULL;
  _BindByPos(qry
    , bindNo
    , direction
    , SQL_C_BINARY
...
JP_INTERNAL(void) JP_Sequence(TJQuery& qry, int32& sequence, const char* sequencer)
{
  ...
  TJQuery q(qry.conn);
  q.command = TJAlloc::newChar(1024);
  switch (cppRetType)
  {
  case CPP_RET_DB2:
    snprintf(q.command, 1024, "select %s.nextval from SYSIBM.SYSDUMMY1 WITH UR;", sequencer);
    break;
  case CPP_RET_ORACLE:
    snprintf(q.command, 1024, "select %s.nextval from dual;", sequencer);
    break;
  default:
    sequence = 0;
    return;
...
JP_INTERNAL(void) JP_BigSequence(TJQuery& qry, SQLLEN& sequence, const char* sequencer)
{
  // This will always get the sequence as char and do the atoll even if the ODBC can handle int64
  char bigsequence[32];
  enum {
    NO_BINDS = 0
    , NO_DEFINES = 1
    , NO_ROWS = 1
    , ROW_SIZE = sizeof(bigsequence)
  };
  TJQuery q(qry.conn);
  q.command = TJAlloc::newChar(1024);
  switch (cppRetType)
  ...
  q.Open(q.command, NO_BINDS, NO_DEFINES, NO_ROWS, ROW_SIZE);
  q.Define(0, (char*)q.data, 32);
  q.Exec();
  if (q.Fetch())
  {
    q.Get(bigsequence, q.data, 32);
    sequence = atoll(bigsequence);
  }
  else
    sequence = 0;
}
...
JP_INTERNAL(void) JP_Date(TJQuery& qry, DATE_STRUCT& clidate, const char* aDate)
{
  char date[9];
...
JP_INTERNAL(void) JP_Time(TJQuery& qry, TIME_STRUCT& clitime, const char* aTime)
{
  char time[7];

...
char *ODecimal::setFormat(const char *format)
{
    char *str = 0;
    /*
    * conver format to ORACLE format
    */
...
{
    Setup();
    setResult(OCINumberFromText(ociError, (const oratext *)str, (ub4)strlen(str), (const oratext *)numberFormat, (ub4)strlen(numberFormat), 0, 0, &number), JP_MARK);
}
...
{
    Setup();
    setFormat(input.numberFormat);
    if (text)
    {
        size_t len = strlen(numberFormat) + 1;
        text = new char [len + 1];
        strncpy(text, input.text, len);
    }
...
char *ODecimal::getText(OFormat &format)
{
    if (text != 0) delete [] text;
    ub4 len = (ub4) strlen(format.numberFormat) + 1;
    text = new char [len + 1];
    char *tmp = getText(text, len, (const char *)format.numberFormat);
    return tmp;
}

char *ODecimal::getText(char *value, ub4 &length, const char *format)
{
    setResult(result, OCINumberToText(ociError, &number, (const oratext *)format, (ub4)strlen(format), 0, 0, &length, (oratext *)value), JP_MARK);
    return value;
}
...
char *ODecimal::getText(const char *format)
{
    ...
    text = new char [len + 1];
    char *tmp = getText(text, len, numberFormat);
    return tmp;
}

char *ODecimal::getText(ODecimal &anumber)
{
...
//}
/*
ODecimal ODecimal::operator =(const char *& rhs)
{
   ODecimal x(&rhs);
  setResult(result, OCINumberAssign(ociError, (OCINumber*)&x.number, &number), JP_MARK);
  return *this;
}
*/
...
ODecimal ODecimal::operator =(const ODecimal &rhs)
{
    ...
    //  this->setFormat(rhs.numberFormat);
...
ODecimal ODecimal::operator +=(const ODecimal &rhs)
{
    setResult(result, OCINumberAdd(ociError, (OCINumber *)&number, (OCINumber *)&rhs.number, &number), JP_MARK);
    return *this;
}
...
ODecimal ODecimal::operator +(const int &arhs)
{
    ODecimal target, rhs(arhs);
    setResult(target.result, OCINumberAdd(ociError, (OCINumber *)&number, (OCINumber *)&rhs.number, &target.number), JP_MARK);
    return target;
}

ODecimal ODecimal::operator +(const double &arhs)
{
    ODecimal target, rhs(arhs);
    setResult(target.result, OCINumberAdd(ociError, (OCINumber *)&number, (OCINumber *)&rhs.number, &target.number), JP_MARK);
    return target;
...
ODecimal ODecimal::operator -=(const ODecimal &rhs)
{
    setResult(result, OCINumberSub(ociError, (OCINumber *)&number, (OCINumber *)&rhs.number, &number), JP_MARK);
    return *this;
}
...
/*
ODecimal::operator char *()
{
  return getText();
}
*/

bool ODecimal::testMe(void)
{
    //ODecimal::format = "99999999999999999999D999999";
    ODecimal num1, num2, num3, tmp;
    //ub4 len;

    num1.setFormat("99999999999999999D99");
    num1 = "12345678901234567.22";
    if (num1 == (ODecimal)"12345678901234567.22")
    {
        printf("True\n");
    }

    num2.setFormat("99999999999999999D99");
    num2 = "12345678901234567.22";

    num3 = num1 + num2;
    printf("%s\n", num3.getText());
    printf("%s\n", (num1 * 2).getText());
    printf("%s\n", (num3 * 2).getText());
    printf("%s\n", (num3 / (ODecimal)2).getText());
    return true;
}


OFormat::OFormat(const char *format)
{
    numberFormat = 0;
    Textformat = 0;
    setFormat(format);
}
char *OFormat::setFormat(const char *format)
{
    char *str = 0;
    /*
    * conver format to ORACLE format
...
OFormat::OFormat(int digit, int decimal)
{
    numberFormat = 0;
    Textformat = 0;
    setFormat(digit, decimal);
}

char *OFormat::setFormat(int digit, int decimal)
{
    if (numberFormat != 0) delete [] numberFormat;
    numberFormat = new char [digit + decimal + 2];
    Textformat = numberFormat;
...
char *OFormat::operator()(ODecimal &ee)
{
    char *rc = 0;
    rc = ee.getText(*this);
    return rc;

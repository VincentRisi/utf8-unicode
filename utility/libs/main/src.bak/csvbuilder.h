#ifndef _CSVBUILDER_H_
#define _CSVBUILDER_H_
...
public:
    ...
    void add(const char *name, char   *value, size_t size, const char *io)
    {
        ...
        setHeader(name);
        work.set(value, size);
        splits_asch(splits, strtrim(work.data), used++);
    }
    ...
    {
        setHeader(name);
        splits_asi(splits, value, used++);
    }
    void add(const char *name, const int64   value)
    {
        setHeader(name);
        splits_asl(splits, (short)value, used++);
    }
    void add(const char *name, const char *value)
    {
        add(name, strlen(value), (const unsigned char *)value);
    }
    void add(const char *name, const size_t len, const unsigned char *value)
    {
        setHeader(name);
        uwork.set(value, len);
        splits_asch(splits, (char*)uwork.data, used++);
    }
    ...
    void addByte(const char* name, const int8* value, const size_t size, const int16 isNull)
    {
        ...
        i8work.set(value, size);
        splits_asch(splits, (char*)i8work.data, used++);
    }
    ...
    {
        add(name, strlen(value), (const unsigned char*)value, isNull);
    }

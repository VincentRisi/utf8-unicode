...
public class TJCStructs //extends Writer
{
  ...
  private static String generatePadding(Field field, int fillerNo)
  {
    switch (field.type)
    {
      ...
      case Field.UTF8:
        return charPadding(field.length * 3 + 1, fillerNo);
      case Field.UNICODE:
        return charPadding((field.length + 1) * 2 + 1, fillerNo);
      case Field.DOUBLE:
      case Field.FLOAT:
  ...
  static String cppInit(Field field)
  {
    String nullSet = "";
    if (isNull(field))
      nullSet = format(" %sIsNull = 0;", field.useName());
    switch (field.type)
    {
      case Field.BOOLEAN:
      case Field.BYTE:
      ...
      case Field.LONG:
        return field.useName() + " = 0;" + nullSet;
      case Field.CHAR:
      case Field.ANSICHAR:
      case Field.TLOB:
      case Field.XML:
      ...
      case Field.TIMESTAMP:
      case Field.AUTOTIMESTAMP:
        return "memset(" + field.useName() + ", 0, sizeof(" + field.useName() + "));" + nullSet;
      case Field.BLOB:
        return "memset(&" + field.useName() + ", 0, sizeof(" + field.useName() + "));";
      case Field.FLOAT:
      case Field.DOUBLE:
        if (field.precision <= 15)
          return field.useName() + " = 0.0;" + nullSet;
        return "memset(" + field.useName() + ", 0, sizeof(" + field.useName() + "));" + nullSet;
    }
  ...
  static String cppVar(Field field)
  {
    switch (field.type)
    {
      ...
      case Field.UTF8:
        return "unsigned char " + field.useName() + "[" + (field.length * 3 + 1) + "]";
      case Field.UNICODE:
        return "unsigned char " + field.useName() + "[" + (field.length * 2 + 2) + "]";
      case Field.USERSTAMP:
      case Field.DATE:
  ...
  static String fromXMLFormat(Field field)
  {
    ...
    switch (field.type)
    {
      ...
      case Field.UTF8:
      case Field.UNICODE:
        return front + "memcpy(" + field.useName() + ", work.data, sizeof(" + field.useName() + "));";
      case Field.BOOLEAN:
      case Field.BYTE:
  ...
  static String toXMLFormat(Field field)
  {
    ...
    switch (field.type)
    {
      ...
      case Field.UTF8:
        return front + "XRec.ampappend(" + field.useName() + ");" + back;
      case Field.UNICODE:
        return "// " + field.useName() + " <unsupported binary type>";
      case Field.BOOLEAN:
      case Field.BYTE:
      case Field.SHORT:
      case Field.INT:

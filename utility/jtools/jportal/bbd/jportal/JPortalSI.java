  ...
  */
    ...
    try
    {
      ...
      try
      {
        ...
        for (int i=0; i<database.flags.size(); i++)
          outData.println(pad("FLAGS", 10)+"\""+ database.flags.elementAt(i) +"\"");
        ...
        for (int i=0; i<database.tables.size(); i++)
          generate(database.tables.elementAt(i), outData, outLog);
        for (int i=0; i<database.views.size(); i++)
          generate(database.views.elementAt(i), outData, outLog);
      }
      finally
      {
        outData.close();
  ...
  static void generate(Table table, PrintWriter outData, PrintWriter outLog)
  {
    ...
    if (table.comments.size() > 0)
      line = pad(line, 56) + table.comments.elementAt(0);
    outData.println(line);
    for (int i = 1; i < table.comments.size(); i++)
      outData.println(pad("", 56) + "** " + table.comments.elementAt(i));
    for (int i = 0; i < table.options.size(); i++)
      outData.println(pad("OPTIONS", 8) + "\"" + table.options.elementAt(i) + "\"");
    for (int i = 0; i < table.fields.size(); i++)
      generate(table.fields.elementAt(i), outData, outLog);
    outData.println();
    for (int i = 0; i < table.grants.size(); i++)
      generate(table.grants.elementAt(i), outData, outLog);
    outData.println();
    for (int i = 0; i < table.keys.size(); i++)
      generate(table.keys.elementAt(i), outData, outLog);
    for (int i = 0; i < table.links.size(); i++)
      generate(table.links.elementAt(i), outData, outLog);
    for (int i = 0; i < table.views.size(); i++)
      generate(table.views.elementAt(i), outData, outLog);
    for (int i = 0; i < table.procs.size(); i++)
      generate(table.procs.elementAt(i), outData, outLog);
    outData.println();
  }
  static void generate(Field field, PrintWriter outData, PrintWriter outLog)
  {
    ...
    for (int i = 1; i < field.comments.size(); i++)
      outData.println(pad("", 56) + "** " + field.comments.elementAt(i));
  }
  ...
  static void generate(Key key, PrintWriter outData, PrintWriter outLog)
  {
    ...
    for (int i = 0; i < key.options.size(); i++)
      outData.println(pad("OPTIONS", 8) + "\"" + key.options.elementAt(i) + "\"");
    if (key.isPrimary)
      outData.println("PRIMARY");
    else if (key.isUnique)
      outData.println("UNIQUE");
    for (int i = 0; i < key.fields.size(); i++)
      outData.println("  " + key.fields.elementAt(i));
    outData.println();
  }
  static void generate(Link link, PrintWriter outData, PrintWriter outLog)
  {
    ...
    for (int i = 0; i < link.fields.size(); i++)
      outData.println("  " + link.fields.elementAt(i));
    outData.println();
  }
  static void generate(View view, PrintWriter outData, PrintWriter outLog)
  {
    ...
    for (int i = 0; i < view.aliases.size(); i++)
      outData.println("  " + view.aliases.elementAt(i));
    outData.println("CODE");
    for (int i = 0; i < view.lines.size(); i++)
      outData.println("  '" + view.lines.elementAt(i) + "'");
  ...
  static void generate(Proc proc, PrintWriter outData, PrintWriter outLog)
  {
    ...
    for (int i=0; i < proc.comments.size(); i++)
      outData.println(pad("**", 56) + proc.comments.elementAt(i));
    for (int i=0; i < proc.options.size(); i++)
      outData.println(pad("OPTIONS", 8) + "'" + proc.options.elementAt(i) + "'");
    if (stdProc)
      return;
    if (proc.inputs.size() > 0)
    {
      ...
      for (int i=0; i < proc.inputs.size(); i++)
        generate(proc.inputs.elementAt(i), outData, outLog);
    }
    if (proc.outputs.size() > 0)
    {
      outData.println("OUTPUT");
      for (int i=0; i < proc.outputs.size(); i++)
        generate(proc.outputs.elementAt(i), outData, outLog);
    }
    proc.isSql = true;
      outData.print("SQL ");
    outData.println("CODE");

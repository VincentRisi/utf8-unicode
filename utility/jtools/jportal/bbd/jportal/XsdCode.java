...
public class XsdCode extends Generator
{
			...
			PrintWriter outLog)
  {
    ...
    for (int i = 0; i < database.tables.size(); i++)
    {
      ...
      {
        String option = table.options.elementAt(j);
          ...
          for (int i = 0; i < dataset.tables.size(); i++)
          {
            ...
            for (int j = 0; j < table.links.size(); j++)
            {
              ...
              if (linkTable != null)
              {
                ...
                {
                  Key key = linkTable.keys.elementAt(k);
                  if (key.isPrimary)
                  {
                ...
                {
                  String fieldName = link.fields.elementAt(f);
                  if (f > 0)
                    outData.print(" ");
                ...
                for (int k = 0; k < linkTable.keys.size(); k++)
                {
                  ...
                  if (key.isPrimary)
                  {
                    ...
                    {
                      String fieldName = key.fields.elementAt(f);
                      if (f > 0)
                        outData.print(" ");
                      outData.print(fieldName);
                    }
    ...
    if (!table.isStoredProc)
      outData.println("");
		...
		if (!found)
		{
			...
			{
				String line = proc.lines.elementAt(i).line.replace(':', '@');
				outData.print(line);
				if (!table.isStoredProc)
					outData.println("");
			}
      ...
      String output, PrintWriter outData, PrintWriter outLog)
  {
    ...
    if (table.isStoredProc)
    {
      for (int f = 0; f < table.procs.size(); f++)
      {
        ...
        if (proc.name.equalsIgnoreCase("fill"))
        {
          defaultProc = proc;
      ...
      {
        Proc proc = table.procs.elementAt(0);
    ...
    {
      Field field = table.fields.elementAt(f);
    ...
    for (int f = 0; f < table.procs.size(); f++)
    {
      ...
      proc = table.procs.elementAt(f);
      for (int i = 0; i < proc.options.size(); i++)
      {
        String option = proc.options.elementAt(i);
        if (option.equalsIgnoreCase("scalar"))
        {
          queryType = "Scalar";
          break;
        }
      ...
      {
        if (procName.substring(0, 6).equalsIgnoreCase("fillby"))
      ...
      String commandType, PrintWriter outData, PrintWriter outLog)
  {
    ...
    if (!table.isStoredProc)
    {
      ...
      if (!name.equals("InsertCommand") && !name.equals("UpdateCommand"))
      {
        ...
        {
          PlaceHolderPairs pair = placeHolder.pairs
              .elementAt(i);
          Field field = pair.field;
          fields.addElement(field);
        }
			...
			String output, PrintWriter outData, PrintWriter outLog)
  {
    ...
    try
    {
      try
      {
        ...
        {
          Proc proc = table.procs.elementAt(p);
          if (proc.isData == true)
            continue;
      ...
      PrintWriter outData, PrintWriter outLog)
  {
    ...
    {
      String field = key.fields.elementAt(i);

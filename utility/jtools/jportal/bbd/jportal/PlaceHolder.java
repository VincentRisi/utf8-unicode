...
public class PlaceHolder implements Serializable
{
  ...
  private void makeCommand()
  {
    ...
    for (int i = 0; i < proc.lines.size(); i++)
    {
      ...
      {
        if (isWithInclude(l.line))
        {
          String withName = l.line.trim().substring(7).trim();
          comma = makeWith(comma, withName);
        }
        else
        {
  ...
  private boolean isWithInclude(String line)
  {
    return line.trim().startsWith("--with ");
  }

  private String makeWith(String comma, String withName)
  {

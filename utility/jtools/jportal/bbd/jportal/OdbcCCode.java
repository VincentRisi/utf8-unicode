  ...
  static private void generateImplementation(Table table, Proc proc)
  {
    ...
    for (int j = 0; j < placeHolder.pairs.size(); j++)
    {
      ...
      if (field.type == Field.BLOB || field.type == Field.TLOB || field.type == Field.IMAGE)
      {
        ...
        TYPEOF = "Utf8";
      writeln(1, "q_." + bind + TYPEOF + "(" + padder("" + j + ",", 4) + cppBind(field, tablename, proc.isInsert) + padder(", " + cppDirection(field), 4) + ((isNull(field)) ? ", &" + field.useName() + "IsNull" : "") + charFieldFlag(field) + ");");
      if (field.type == Field.BLOB || field.type == Field.TLOB || field.type == Field.IMAGE)
    ...
    else
    {
      for (int j = 0; j < proc.outputs.size(); j++)
      {
        ...
        if (field.type == Field.BLOB || field.type == Field.TLOB || field.type == Field.IMAGE)
          ...
          TYPEOF = "Utf8";
        writeln(1, "q_.Define" + TYPEOF + "(" + padder("" + j + ",", 4) + cppDefine(field) + ");");
      }
    }
    writeln(1, "q_.Exec();");

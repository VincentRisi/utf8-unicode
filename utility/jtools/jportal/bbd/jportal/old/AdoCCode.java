...
public class AdoCCode extends Generator
{
  ...
  public static Vector flags()
  {
    if (flagsVector == null)
    {
      ...
      flagDefaults();
      flagsVector.addElement(new Flag("sqlprocs", sqlprocs, "Generate Sql Procs"));
      flagsVector.addElement(new Flag("mssql", mssql, "Generate MSSql"));
    }
    return flagsVector;
  }
  ...
  static void generateImplementation(Table table, Proc proc, PrintWriter outData)
  {
    ...
    if (proc.placeHolders.size() > 0)
      for (int j = 0; j < proc.placeHolders.size(); j++)
      {
        ...
        Field field = proc.inputs.elementAt(n);
  ...
  public static void generateVB6Structs(Table table, PrintWriter outData)
  {
    ...
    for (int i = 0; i < table.procs.size(); i++)
    {
      ...
      if (proc.hasDiscreteInput())
      {
        ...
        {
          String s = proc.dynamics.elementAt(j);
          Integer ds = proc.dynamicSizes.elementAt(j);

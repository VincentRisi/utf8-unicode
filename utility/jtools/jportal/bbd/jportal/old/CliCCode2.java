...
public class CliCCode2 extends Generator
{
    ...
    try
    {
      ...
      try
      {
        ...
        try
        {
          ...
          writeln("#ifndef _" + table.useName().toLowerCase() + "SH_MARSHAL_VERSION");
          writeln("#define _" + table.useName().toLowerCase() + "SH_MARSHAL_VERSION");
          writeln();
          writeln("#include <stddef.h>");
          writeln("#include \"padgen.h\"");
          writeln("#include \"cliapi.h\"");
          writeln("#include \"foldbuilder.h\"");
          writeln();
  ...
  static void generateUserOutputRecs(Table table)
  {
    for (int i = 0; i < table.procs.size(); i++)
    {
      ...
      if (fields.size() > 0)
      {
        ...
        writeln();
        needsFold = headerSwaps("", fields, null, needsFold);
      ...
      if (proc.hasDiscreteInput())
      {
        ...
        headerSwaps(baseClass, inputs, proc, needsFold);
        String useName = table.useName() + proc.upperFirst();
  ...
  private static boolean headerSwaps(String baseClass, Vector inputs, Proc proc, boolean setFold)
  {
    ...
    writeln(1, "}");
    writeln(1, "static bool NeedsFold() { return " + (needsFold ? "true" : "false") + "; }");
    return needsFold;
  }
  ...
  private static void extendDataBuildHeader(String baseClass, Vector inputs, String useName, Vector dynamics, Proc proc)
  {
    ...
    for (int j = 0; j < inputs.size(); j++)
    {
      ...
      if (field.type == Field.BLOB)
        writeln(2, "dBuild.add(\"" + field.useName() + "\"" + maxlenAdd(field) + ", " + field.useName() + ".len, " + field.useName() + ".data" + nullAdd(field) + ");");
      else
        writeln(2, "dBuild.add(\"" + field.useName() + "\"" + maxlenAdd(field) + ", " + field.useName() + nullAdd(field) + ");");
    }
    for (int j = 0; j < dynamics.size(); j++)
    {
      String str = (String)dynamics.elementAt(j);
      Integer n = proc.dynamicSizes.elementAt(j);
      writeln(2, "dBuild.add(\"" + str + "\", " + (n.intValue() + 1) + ", " + str + ");");
    }
    writeln(1, "}");
    ...
    for (int j = 0; j < inputs.size(); j++)
    {
      ...
      if (proc != null && proc.hasOutput(field.name))
        continue;
      if (field.type == Field.BLOB)
        writeln(2, "dBuild.set(\"" + field.useName() + "\", " + field.useName() + ".len, " + field.useName() + ".data, " + field.length + nullSet(field) + ");");
      else
  ...
  static void generateMultipleImplementation(Table table, Proc proc)
  {
    ...
    for (int i = 0; i < proc.inputs.size(); i++)
    {
      ...
      if (isNull(field))
        ...
        writeln(2, "" + field.useName() + "IsNull[i] = Recs[i]." + field.useName() + ".length() == 0 ? JP_NULL : SQL_NTS;");
      else if (field.type == Field.ANSICHAR && field.isNull == true)
  ...
  static void generateImplementation(Table table, Proc proc)
  {
    ...
    for (int j = 0; j < placeHolder.pairs.size(); j++)
    {
      ...
      if (field.type == Field.BLOB)
        writeln(1, "q_.BindBlob(" + padder("" + j + ",", 4) + cppBind(field, tablename, proc.isInsert) + padder(", " + cppDirection(field), 4) + ((isNull(field)) ? ", &" + field.useName() + "IsNull" : "") + charFieldFlag(field) + ");");
      else
    ...
    for (int j = 0; j < proc.outputs.size(); j++)
    {
      ...
      if (field.type == Field.BLOB)
        writeln(1, "q_.DefineBlob(" + padder("" + j + ",", 4) + cppDefine(field) + ");");
      else

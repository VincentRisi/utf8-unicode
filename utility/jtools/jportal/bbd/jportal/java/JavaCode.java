...
public class JavaCode extends Generator
{
  ...
  public static Vector flags()
  {
    if (flagsVector == null)
    {
      ...
      flagsVector.addElement(new Flag("add annotates", new Boolean(addAnnotates), "Add String size annotations"));
    }
    return flagsVector;
  }
  ...
  {
    if (flagsVector != null)
      addAnnotates = toBoolean(flagsVector.elementAt(0));
    else
      flagDefaults();
    for (int i = 0; i < database.flags.size(); i++)
    {
      String flag = database.flags.elementAt(i);
      if (flag.equalsIgnoreCase("add annotates"))
        addAnnotates = true;
    }
    if (addAnnotates)
      outLog.println(" (add annotates)");
  }
  ...
  private static void generateOtherCopiers(Table table, Proc proc, PrintWriter outData)
  {
    ...
    {
      Field field = proc.inputs.elementAt(jj);
      String name = field.useLowerName();
      outData.println("    _result." + name + " = " + name + ";");
      if (field.isNull == true && field.isEmptyOrAnsiAsNull() == false)
        outData.println("    _result." + name + "IsNull = " + name + "IsNull;");
    ...
    {
      Field field = proc.outputs.elementAt(jj);
    ...
    {
      String s = proc.dynamics.elementAt(jj);
  ...
  private static void emitProc(Proc proc, PrintWriter outData)
  {
    ...
    {
      if (proc.noRows > 0)
        outData.println(indent + "    " + prep + ".setFetchSize(" + proc.noRows + ");");
  ...
  private static String javaVar(Field field)
  {
    ...
    switch (field.type)
    {
      ...
      case Field.CHAR:
        if (addAnnotates == true)
          annotate = "@FieldAnnotate(size=" + (field.length + 1) + ") ";
        return annotate + "String " + name;
      case Field.DATE:
        if (addAnnotates == true)
          annotate = "@FieldAnnotate(size=9,type=FieldType.DATE) ";
        return annotate + "String " + name;
      case Field.DATETIME:
        if (addAnnotates == true)
          annotate = "@FieldAnnotate(size=15,type=FieldType.DATETIME) ";
        return annotate + "String " + name;
      case Field.TIME:
        if (addAnnotates == true)
          annotate = "@FieldAnnotate(size=7,type=FieldType.TIME) ";
        return annotate + "String " + name;
      case Field.TIMESTAMP:
        if (addAnnotates == true)
          annotate = "@FieldAnnotate(size=15,type=FieldType.TIMESTAMP) ";
        return annotate + "String " + name;
      case Field.FLOAT:
      case Field.DOUBLE:
        ...
        {
          if (addAnnotates == true)
            annotate = "@FieldAnnotate(size=" + (field.precision + 3) + ",type=FieldType.MONEY) ";
          return annotate + "String " + name;
        }
        else
          return "double " + name;
      ...
      case Field.TLOB:
        if (addAnnotates == true)
          annotate = "@FieldAnnotate(size=" + (field.length + 1) + ",type=FieldType.TLOB) ";
        return annotate + "String " + name;
      case Field.MONEY:
        if (addAnnotates == true)
          annotate = "@FieldAnnotate(size=21,type=FieldType.MONEY) ";
        return annotate + "String " + name;
      case Field.USERSTAMP:
        if (addAnnotates == true)
          annotate = "@FieldAnnotate(size=" + (field.length + 1) + ",type=FieldType.USERSTAMP) ";
        return annotate + "String " + name;
    }
    return "unknown";
  }

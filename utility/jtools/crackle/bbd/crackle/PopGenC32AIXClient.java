...
public class PopGenC32AIXClient extends Generator
{
  ...
  public static void generateCClientHeader(Module module, String output, PrintWriter outLog)
  {
    try
    {
      ...
      try
      {
        ...
        outData.println("  int getHandle();");
        outData.println("  void Logon(char* UserID, char* Service, char* Host, int Timeout=150000);");
        ...
        outData.println("  char* ErrBuffer(char *Buffer, int BufferLen);");
        outData.println("  char* ErrorDesc(char *Buffer, int BufferLen);");
  ...
  public static void generateCClient(Module module, String output, PrintWriter outLog)
  {
    try
    {
      ...
      try
      {
        ...
        outData.println("#include \"snaprpcgnuclient.h\"");
                ...
                || parameter.type.reference == Type.BYREFPTR))
            {
              ...
              ||  prototype.hasInputSize(parameter.name))
              {
                if (parameter.type.reference == Type.BYREFPTR)
                  outData.println("  "+parameter.type.c32DefRefPAsP(prototype.name+parameter.name, false)+";");
                else
                  outData.println("  "+parameter.type.c32Def(prototype.name+parameter.name, false)+";");
              }
            }
          }
        }
        outData.println("  t"+module.name+"SnapCB(tString UserID, tString Service, tString Host, int Timeout)");
        w1 = ":";
        ...
        outData.println();
        outData.println("e"+module.name+" "+module.name+"SnapLogon(int* Handle, char* UserID, char* Service, char* Host, int Timeout)");
        outData.println("{");
        outData.println("  try");
        outData.println("  {");
        ...
        outData.println("}");
        outData.println();
        outData.println("e"+module.name+" "+module.name+"SnapLogoff(int* Handle)");
        outData.println("{");
        ...
        outData.println("}");
        outData.println();
        outData.println("e"+module.name+" "+module.name+"SnapErrBuffer(int Handle, char *Buffer, int BufferLen)");
        outData.println("{");
        outData.println("  if (Handle < CookieStart || Handle >= CookieStart+NoCookies)");
        outData.println("    return "+module.name.toUpperCase()+"_INV_COOKIE;");
        ...
        outData.println("}");
        outData.println();
        outData.println("void "+module.name+"SnapErrorDesc(e"+module.name+" RC, char *Buffer, int BufferLen)");
        outData.println("{");
        ...
        outData.println("}");
        outData.println();
        outData.println("void "+module.name+"SnapVersion(char *Buffer, int BufferLen)");
        outData.println("{");
        ...
        outData.println("int t"+module.name+"::getHandle()");
        outData.println("{");
        outData.println("  if (Handle < CookieStart || Handle >= CookieStart+NoCookies)");
        ...
        outData.println("}");
        outData.println();
        outData.println("void t"+module.name+"::Logon(char* UserID, char* Service, char* Host, int Timeout)");
        outData.println("{");
        ...
        outData.println("}");
        outData.println();
        outData.println("char* t"+module.name+"::ErrBuffer(char *Buffer, int BufferLen)");
        outData.println("{");
        ...
        outData.println("}");
        outData.println();
        outData.println("char* t"+module.name+"::ErrorDesc(char *Buffer, int BufferLen)");
        outData.println("{");
  ...
  */
  ...
  {
    String w1 = "";
    outData.print(prototype.type.c32Def("t"+module.name+"::"+prototype.name, false)+"(");
    for (int i = 0; i < prototype.parameters.size(); i++)
    {
      Field parameter = (Field) prototype.parameters.elementAt(i);
      outData.print(w1 + parameter.type.c32Def(parameter.name, false));
      w1 = ", ";
    }
    outData.println(")");
    outData.println("{");
    ...
    ||  prototype.type.reference == Type.BYPTR)
      outData.println("  "+prototype.type.c32Name(false)+" Result;");
  ...
  */
  public static void generateCClient(Module module, Prototype prototype, int no, PrintWriter outData)
  {
    ...
    &&  prototype.type.typeof != Type.VOID)
      hasReturn = true;
    outData.print("e"+module.name+" "+module.name+"Snap" + prototype.name+"(int Handle, int Signature");
    if (hasReturn)
        outData.print(", "+prototype.type.c32Name(false)+"& Result");
    for (int i = 0; i < prototype.parameters.size(); i++)
    {
      Field parameter = (Field) prototype.parameters.elementAt(i);
      outData.print(", " + parameter.type.c32Def(parameter.name, false));
    }
    outData.println(")");
    outData.println("{");
      ...
      ||  field.type.reference == Type.BYREFPTR)
      {
        if (input.hasSize() == false)
        {
          if (field.type.typeof == Type.CHAR)
            outData.println("    snapCB->sendBuffLen += (sizeof(int)+strlen("+field.name+")+1);");
          else
        ...
        else
        {
          ...
          outData.println("    snapCB->sendBuffLen += (sizeof(int)+"+op.name+"*sizeof(*"+field.name+"));");
        }
      }
      else
      {
    ...
    if (prototype.inputs.size() == 0)
      w1 = "// ";
    outData.println("    *(int*)ip = Signature;");
    outData.println("    SwapBytes(*(int*)ip);");
    outData.println("    "+w1+"ip += sizeof(int);");
    for (int i = 0; i < prototype.inputs.size(); i++)
    {
      ...
      ||  field.type.reference == Type.BYREFPTR)
      {
        if (input.hasSize() == false)
        {
          if (field.type.typeof == Type.CHAR)
          {
            outData.println("    *(int*)ip = (strlen("+field.name+")+1);");
            outData.println("    SwapBytes(*(int*)ip);");
            outData.println("    ip += sizeof(int);");
            outData.println("    memcpy(ip, "+field.name+", (int)strlen("+field.name+")+1);");
            outData.println("    "+w1+"ip += (strlen("+field.name+")+1);");
          }
          else
          {
            outData.println("    memcpy(ip, (void*)"+field.name+", (int)sizeof(*"+field.name+"));");
            outData.println("    "+w1+"ip += sizeof(*"+field.name+");");
          }
        }
        else
        {
          outData.println("    *(int*)ip = ("+op.name+"*sizeof(*"+field.name+"));");
          outData.println("    SwapBytes(*(int*)ip);");
          outData.println("    ip += sizeof(int);");
          outData.println("    memcpy(ip, (void*)"+field.name+", (int)("+op.name+"*sizeof(*"+field.name+")));");
          outData.println("    "+w1+"ip += (int)("+op.name+"*sizeof(*"+field.name+"));");
        }
      }
      else
      {
        outData.println("    memcpy(ip, (char*)&"+field.name+", (int)sizeof("+field.name+"));");
        outData.println("    "+w1+"ip += sizeof("+field.name+");");
      }
      generateCSwaps(module, prototype, field, op, outData);
    }
    ...
    if (prototype.outputs.size() > 0 || hasReturn)
    {
      ...
      if (hasReturn)
      {
        outData.println("    memcpy(&Result, ip, (int)sizeof(Result));");
      ...
      for (int i = 0; i < prototype.outputs.size(); i++)
      {
        ...
        ||  field.type.reference == Type.BYREFPTR)
        {
          ...
          {
            outData.println("    snapCB->recvSize = *(int*)ip;");
            ...
            if (field.type.reference == Type.BYREFPTR)
            {
              ...
              if (sf.type.reference == Type.BYPTR)
                w = "*";
              outData.println("    "+field.name+" = new "+field.type.c32Name(false)+"["+w+s+"];");
            }

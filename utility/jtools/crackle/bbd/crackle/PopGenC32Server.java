...
public class PopGenC32Server extends Generator
{
  ...
  private static void generateCHeader(Module module, String output, PrintWriter outLog)
  {
    try
    {
      ...
      try
      {
        ...
        PopGenC32.generateC32Externs(module, outData);
        outData.println("extern char *"+module.name+"Version;");
        outData.println("extern int "+module.name+"Signature;");
        outData.println("extern char *ConfigFile;");
        outData.println();
        PopGenC32.generateC32Structs(module, outData, true);
        outData.println();
        ...
        for (int i = 0; i < module.structures.size(); i++)
        {
          ...
          for (int j=0; j < structure.fields.size(); j++)
          {
            ...
            outData.println("  "+field.type.c32Def(field.name, false)+";");
          }
        }
        for (int i = 0; i < module.prototypes.size(); i++)
        {
          ...
          if (prototype.codeType != Prototype.PRIVATE)
            continue;
          PopGenC32.generateC32Header(module, prototype, outData);
        }
        ...
        outData.println("  int Result;");
        outData.println("  int ReplySize;");
        outData.println("  int RecvSize;");
    ...
    try
    {
      ...
      try
      {
        ...
        if (doConnect == true && doDBFill == true)
        {
          ...
          outData.println("  int Limited(REC* List, int Limit)");
          outData.println("  {");
          ...
          outData.println("  }");
          outData.println("  int Dynamic(REC*& List)");
          outData.println("  {");
          outData.println("    int Count = 0;");
          outData.println("    while (Fetch())");
          outData.println("      AddList(List, Count, Rec, 16);");
          outData.println("    return Count;");
          outData.println("  }");
          outData.println("  int DynamicLimited(REC*& List, int Limit)");
          outData.println("  {");
          outData.println("    int Count = 0;");
          outData.println("    while (Count < Limit && Fetch())");
          outData.println("      AddList(List, Count, Rec, 16);");
        ...
        for (int i = 0; i < module.prototypes.size(); i++)
        {
          ...
          {
            PopGenC32.generateC32ImplCode(module, prototype, outData);
            outData.println("{");
  ...
  private static boolean generateCServer(Module module, String output, PrintWriter outLog)
  {
    ...
    try
    {
      ...
      try
      {
        ...
        outData.println("int "+module.name+"Signature = "+module.signature+";");
        outData.println();
        ...
        if (alignForSun == true)
        {
          outData.println("inline int alignData(int size)");
          outData.println("{");
        ...
        outData.println("        Signature = *(int*)ip;");
        outData.println("        SwapBytes(Signature);");
        if (alignForSun == true)
          outData.println("        ip += alignData(sizeof(int));");
        else
          outData.println("        ip += sizeof(int);");
        ...
        for (int i = 0; i < module.prototypes.size(); i++)
        {
          ...
          outData.println("            Result = (int)"+module.name.toUpperCase()+"_INV_SIGNATURE;");
          outData.println("          else");
          outData.println("            Result = (int)"+prototype.name+"(ip);");
          outData.println("          LogFile->lprintf(eLogDebug, \"Result=%d "+prototype.name+"["+prototype.signature(true)+"]\", Result);");
          outData.println("          break;");
        }
        outData.println("        default:");
        outData.println("          Result = (int)"+module.name.toUpperCase()+"_UNKNOWN_FUNCTION;");
        outData.println("        }");
        ...
        if (doConnect == true)
        {
          ...
          outData.println("        Result=(int)"+module.name.toUpperCase()+"_UNCAUGHT_DB_ERROR;");
          outData.println("      }");
        }
        outData.println("    }");
        if (hasShutDownCode)
  ...
  */
  private static void generateCServer(Module module, Prototype prototype, int no, PrintWriter outData)
  {
    ...
    for (int i = 0; i < prototype.inputs.size(); i++)
    {
      ...
      ||  field.type.reference == Type.BYREFPTR)
      {
        if (input.hasSize() == false)
        {
          ...
          {
            outData.println("    RecvSize = *(int*)ip;");
          ...
          {
            outData.println("    "+field.type.c32Name(false)+"* "+field.name+" = ("+field.type.c32Name(false)+"*)ip;");
            if (addUp)
            {
              if (alignForSun == true)
    ...
    ||  prototype.type.reference == Type.BYPTR)
    {
      ...
      {
        if (alignForSun == true)
          outData.println("    ReplySize += alignData((int)sizeof("+prototype.type.c32Name(false)+")); // Result Size");
        else
          outData.println("    ReplySize += (int)sizeof("+prototype.type.c32Name(false)+"); // Result Size");
        hasOutput = true;
        hasResult = true;
      }
    }
    ...
    for (int i = 0; i < prototype.outputs.size(); i++)
    {
      ...
      if (field.type.reference == Type.BYPTR)
      {
        if (output.hasSize() == false)
        {
          ...
          {
            if (alignForSun == true)
              outData.println("    ReplySize += alignData(sizeof("+field.type.c32Name(false)+")); // size "+field.name);
            else
              outData.println("    ReplySize += sizeof("+field.type.c32Name(false)+"); // size "+field.name);
          }
        }
        else
        {
          ...
          if (op.isConstant == false)
            w1 = "*";
          if (alignForSun == true)
            outData.println("    ReplySize += alignData(sizeof(int));  // size of Block");
          else
            outData.println("    ReplySize += sizeof(int);  // size of Block");
          if (alignForSun == true)
            outData.println("    ReplySize += alignData("+w1+""+op.name+" * sizeof("+field.type.c32Name(false)+")); // size "+field.name);
          else
            outData.println("    ReplySize += ("+w1+""+op.name+" * sizeof("+field.type.c32Name(false)+")); // size "+field.name);
        }
      }
      else if (field.type.reference == Type.BYREFPTR)
      {
        ...
        else
        {
          Variants++;
          if (alignForSun == true)
            outData.println("    ReplySize += alignData(sizeof("+field.type.c32Name(false)+"*)); // Variant "+field.name);
          else
            outData.println("    ReplySize += sizeof("+field.type.c32Name(false)+"*); // Variant "+field.name);
        }
      }
      else
      {
    ...
    if (hasOutput)
    {
      if (Variants > 0)
      {
        outData.println("    int Variants["+Variants+"];");
        outData.println("    int VariantSize["+Variants+"];");
        Variants = 0;
      }
    ...
    ||  prototype.type.reference == Type.BYPTR)
    {
      if (prototype.type.reference != Type.BYPTR)
      {
        outData.println("    "+prototype.type.c32Name(false)+"* Result = ("+prototype.type.c32Name(false)+"*) ip;");
        if (prototype.outputs.size() > 0)
        {
          if (alignForSun == true)
            outData.println("    ip += alignData(sizeof("+prototype.type.c32Name(false)+"));");
          else
            outData.println("    ip += sizeof("+prototype.type.c32Name(false)+");");
        }
      }
    }
    for (int i = 0; i < prototype.outputs.size(); i++)
    {
      ...
      if (field.type.reference == Type.BYPTR)
      {
        if (output.hasSize() == false)
        {
          ...
          else
          {
            if (input != null)
            {
              outData.println("    memcpy(ip, "+field.name+", sizeof("+field.type.c32Name(false)+"));");
              outData.println("    "+field.name+" = ("+field.type.c32Name(false)+"*)ip;");
            }
            else
              outData.println("    "+field.type.c32Name(false)+"* "+field.name+" = ("+field.type.c32Name(false)+"*)ip;");
            if (addUp)
            {
              if (alignForSun == true)
                outData.println("    ip += alignData(sizeof("+field.type.c32Name(false)+"));");
              else
                outData.println("    ip += sizeof("+field.type.c32Name(false)+");");
            }
          }
        }
        else
        {
          ...
          if (op.isConstant == false)
            w1 = "*";
          outData.println("    *(int*)ip = ("+w1+""+op.name+" * sizeof("+field.type.c32Name(false)+"));");
          outData.println("    SwapBytes(*(int*)ip);");
          if (alignForSun == true)
            outData.println("    ip += alignData(sizeof(int));");
          else
            outData.println("    ip += sizeof(int);");
          if (input != null)
          {
            outData.println("    memcpy(ip, "+field.name+", ("+w1+op.name+" * sizeof("+field.type.c32Name(false)+")));");
            outData.println("    "+field.name+" = ("+field.type.c32Name(false)+"*)ip;");
          }
          else
            outData.println("    "+field.type.c32Name(false)+"* "+field.name+" = ("+field.type.c32Name(false)+"*)ip;");
          ...
          {
            if (alignForSun == true)
              outData.println("    ip += alignData("+w1+op.name+" * sizeof("+field.type.c32Name(false)+"));");
            else
              outData.println("    ip += ("+w1+op.name+" * sizeof("+field.type.c32Name(false)+"));");
          }
        }
      }
      else if (field.type.reference == Type.BYREFPTR)
      {
        ...
        else
        {
          ...
          if (input != null)
            outData.println("    ("+field.type.c32Name(false)+"*)*ip = "+field.name+";");
          else
            outData.println("    "+field.type.c32Name(false)+"** "+field.name+" = ("+field.type.c32Name(false)+"**)ip;");
          if (addUp)
          {
    ...
    if (Variants > 0)
    {
      ...
      outData.println("    ip = ReplyBody;");
      outData.println("    int tail = ReplySize;");
        ...
        ||  field.type.reference == Type.BYREFPTR)
        {
          ...
          else
          {
            ...
            if (field.type.reference == Type.BYREFPTR)
              outData.println("    VariantSize["+(VariantSize++)+"] = ("+w1+""+op.name+" * sizeof("+field.type.c32Name(false)+"));");
          }
        }
        else
        {
      ...
      outData.println("    int pos = 0;");
      outData.println("    {");
      outData.println("      for (int i = 0; i < "+Variants+"; i++)");
      outData.println("      {");
      outData.println("        int size = Variants[i]-pos;");
      ...
      outData.println("        SwapBytes(*(int*)op);");
      if (alignForSun == true)

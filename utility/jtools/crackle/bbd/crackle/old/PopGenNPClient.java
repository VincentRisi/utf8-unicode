  ...
  public static void generateCClientHeader(Module module, String output, PrintWriter outLog)
  {
    try
    {
      ...
      try
      {
        ...
        outData.println();
        outData.println("#ifndef DLLEXPORT_STDCALL");
        outData.println("#  ifdef M_W32");
        outData.println("#    define DLLEXPORT_STDCALL(type) __declspec(dllexport) type __stdcall");
        outData.println("#  else");
        outData.println("#    define DLLEXPORT_STDCALL(type) type");
        outData.println("#  endif");
        outData.println("#endif DLLEXPORT_STDCALL");
        outData.println();
  ...
  public static void generateCClient(Module module, String output, PrintWriter outLog)
  {
    try
    {
      ...
      try
      {
        ...
        outData.println();
        outData.println("extern \"C\" DLLEXPORT_STDCALL(e"+module.name+") "+module.name+"SnapLogon(int32* Handle, char* Pipename, int32 Timeout)");
        outData.println("{");
        outData.println("  try");
        outData.println("  {");
        ...
        outData.println("}");
        outData.println();
        outData.println("extern \"C\" DLLEXPORT_STDCALL(e"+module.name+") "+module.name+"SnapLogoff(int32* Handle)");
        outData.println("{");
        ...
        outData.println("}");
        outData.println();
        outData.println("extern \"C\" DLLEXPORT_STDCALL(e"+module.name+") "+module.name+"SnapErrBuffer(int32 Handle, char *Buffer, int32 BufferLen)");
        outData.println("{");
        outData.println("  if (Handle < CookieStart || Handle >= CookieStart+NoCookies)");
        outData.println("    return "+module.name.toUpperCase()+"_INV_COOKIE;");
        ...
        outData.println("}");
        outData.println();
        outData.println("extern \"C\" DLLEXPORT_STDCALL(void) "+module.name+"SnapErrorDesc(e"+module.name+" RC, char *Buffer, int32 BufferLen)");
        outData.println("{");
        ...
        outData.println("}");
        outData.println();
        outData.println("extern \"C\" DLLEXPORT_STDCALL(void) "+module.name+"SnapVersion(char *Buffer, int32 BufferLen)");
        outData.println("{");
  ...
  */
  public static void generateCClient(Module module, Prototype prototype, int no, PrintWriter outData)
  {
    ...
    &&  prototype.type.typeof != Type.VOID)
      hasReturn = true;
    outData.print("extern \"C\" DLLEXPORT_STDCALL(e"+module.name+") "+module.name+"Snap" + prototype.name+"(int32 Handle, int32 Signature");
    if (hasReturn)
    ...
    if (submittals.size() > 0)
    {
      for (int i = 0; i < submittals.size(); i++)
      {
        ...
        outData.println("extern \"C\" DLLEXPORT_STDCALL(void) "+module.name+"Snap"+prototype.name+parameter.name+"Prepare(int32 Handle, int32 Size)");
        outData.println("{");
        outData.println("  if (Handle < CookieStart || Handle >= CookieStart+NoCookies)");
        ...
        outData.println("}");
        outData.println();
        outData.print("extern \"C\" DLLEXPORT_STDCALL(int32) "+module.name+"Snap"+prototype.name+parameter.name+"Fill(int32 Handle");
    ...
    {
      outData.print("extern \"C\" DLLEXPORT_STDCALL(e"+module.name+") "+module.name+"Snap" + prototype.name + "Start(int32 Handle, int32 Signature");
      if (hasReturn)
        outData.print(", "+prototype.type.cName(false)+"& Result");
      for (int i = 0; i < prototype.parameters.size(); i++)
      {
...
//        outData.println("  if (snapCB->"+prototype.name+parameter.name+" == 0)");
    ...
    if (retrievals.size() > 0)
    {
      ...
      {
        Field parameter = (Field) retrievals.elementAt(i);
        outData.print("extern \"C\" DLLEXPORT_STDCALL(int32) "+module.name+"Snap"+prototype.name+parameter.name+"Next(int32 Handle");
        ...
        outData.println("}");
        outData.println();
        outData.println("extern \"C\" DLLEXPORT_STDCALL(void) "+module.name+"Snap"+prototype.name+parameter.name+"Done(int32 Handle)");
        outData.println("{");
        outData.println("  if (Handle < CookieStart || Handle >= CookieStart+NoCookies)");
        outData.println("    return;");
        outData.println("  tSnapCB* snapCB = CBHandle.Use(Handle);");

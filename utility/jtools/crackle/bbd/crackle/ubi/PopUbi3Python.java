...
public class PopUbi3Python extends Generator
{
   ...
   *  Static constructor
   */
  private static final boolean useAsImport = true;
  /**
  ...
  */
  ...
  private static void assertof(PythonArgs arg)
  {
    String x = "";
    switch (arg.field.type.typeof)
    {
      case Type.CHAR:
      case Type.STRING:
        writeln(1, format("if isinstance(%1$s, bytes): %1$s = %1$s.decode()", arg.name));
        break;
    }
    switch (arg.field.type.typeof)
    {
    ...
    writeln(1, format("if not isinstance(%1$s, %2$s): raise AssertionError('%1$s is not an instance of %2$s')",arg.name, x));
  }
